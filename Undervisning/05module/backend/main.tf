...
resource "azurerm_resource_group" "ez-rg-backend" {
  name     = var.rg_backend_name
  location = var.location[0]
}

resource "azurerm_storage_account" "sa_backend" {
  name                     = "${var.saac_backend_name}${random_string.random_string.result}"
  resource_group_name      = azurerm_resource_group.ez-rg-backend.name
  location                 = azurerm_resource_group.ez-rg-backend.location
